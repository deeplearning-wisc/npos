...
from models.resnet_im100 import resnet101
...
parser.add_argument('--start_epoch_KNN', type=int, default=300, help='start epoch to use the outlier loss')
...
def set_model(args):
    model = resnet101(num_class=num_classes)
...
def main():
    ...
    criterion_disp = DispLoss(args, model, train_loader, temperature=args.temp, cifar=False).cuda()
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if sum_temp == args.n_cls * args.sample_number and epoch < args.start_epoch_KNN:
            ...
            if len(ood_samples) != 0:
                ...
                energy_score_for_fg = model(penultimate, mlp=True)
                energy_score_for_bg = model(ood_samples, mlp=True)
