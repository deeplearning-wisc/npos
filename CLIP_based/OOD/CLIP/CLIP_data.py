...
class CLIP_model(nn.Module):
    def __init__(self, num_classes=100, layers=8,  type='RN101', device='cuda', T=0.1):
    ...
    def forward(self, input, fc=False):
        if fc==True:
            ...
            score = self.uncertainty(input)
            return output, score

        feature = self.model(input).float()
        output = self.logits_compute(feature)
        score = self.uncertainty(feature)
