...
def sample_estimator(model, num_classes, feature_list, train_loader):
    ...
    for data, target in train_loader:
        ...
        for i in range(num_output):
            ...
            #TEMP
            # if i == 4: 
            #     out_features[i] = out_features[i] / out_features[i].norm(p=2, dim=1, keepdim=True)
...
def get_Mahalanobis_score(inputs, model, num_classes, sample_mean, precision, num_output, magnitude):
    '''
    e.g. when layer_index = 0, out_features.shape: 80, 24, 32, 32
         -- after flatten--> 80, 24, 1024 --after taken mean --> shape: 80, 24 
    i.e. channel info is reserved
    '''
    for layer_index in range(num_output):
        data = Variable(inputs, requires_grad = True)
        data = data.cuda() #shape 10,3,32,32
        ...
        #TEMP
        # if layer_index  == 4: 
        #         out_features = out_features / out_features.norm(p=2, dim=1, keepdim=True)

        gaussian_score = 0
        for i in range(num_classes):
            batch_sample_mean = sample_mean[layer_index][i]
            zero_f = (out_features.data - batch_sample_mean).double()
        ...
        zero_f = (out_features - batch_sample_mean).double()
        ...
        noise_out_features = model.encoder.intermediate_forward(Variable(tempInputs), layer_index)
        # noise_out_features = model.module.intermediate_forward(Variable(tempInputs), layer_index)
        noise_out_features = noise_out_features.view(noise_out_features.size(0), noise_out_features.size(1), -1)
        noise_out_features = torch.mean(noise_out_features, 2)
        #TEMP
        # if layer_index == 4: 
        #         noise_out_features = noise_out_features / noise_out_features.norm(p=2, dim=1, keepdim=True)
        noise_gaussian_score = 0
        for i in range(num_classes):
            batch_sample_mean = sample_mean[layer_index][i]
            zero_f = (noise_out_features.data - batch_sample_mean).double()
            term_gau = -0.5*torch.mm(torch.mm(zero_f, precision[layer_index]), zero_f.t()).diag()
            if i == 0:
