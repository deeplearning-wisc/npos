...
class SupCEHeadResNet(nn.Module):
    ...
    def __init__(self, name='resnet50', head='mlp', feat_dim=128, num_classes=10, multiplier=1, pelu=512):
        super(SupCEHeadResNet, self).__init__()
        model_fun, dim_in = model_dict[name]
        self.encoder = model_fun()
        self.mlp = nn.Sequential(
                nn.Linear(pelu, feat_dim),
                nn.ReLU(inplace=True),
        ...
        # END
        self.prototypes = nn.Parameter(torch.zeros(num_classes,feat_dim), requires_grad=True)
        if head == 'linear':
            self.head = nn.Linear(dim_in, feat_dim)
        elif head == 'mlp':
            self.head = nn.Sequential(
    ...
    def forward(self, x):
        features = self.encoder(x)
        unnorm_features = self.head(features)
    ...
    def init_class_prototypes(self, args, train_loader):
        ...
        with torch.no_grad():
            ...
            for idx, (input, target) in enumerate(train_loader):
                ...
                penultimate = self.encoder(input)
                features= F.normalize(self.head(penultimate), dim=1)
