...
net.load_state_dict({k.replace('module.', ''): v for k, v in torch.load('/nobackup-slow/taoleitian/model/vos/ImageNet-100/MCM/norm/0.1/ImageNet-100_dense_baseline_dense_epoch_29.pt').items()}, strict=False)
...
def train(epoch, ood_list, len_queue):
    ...
    for idx, (data, target) in enumerate(train_loader):
        ...
        if len_queue >= 100000 and epoch < args.start_epoch:
            data_dict = torch.cat((data_dict[len(output):],
                                             output.detach()), 0)
            sum_temp += len(output)
            ...
            for index in range(10):
                sample_index = np.random.choice(data_dict.shape[0], int(1000),
                                                replace=False)

                ID = data_dict[sample_index].detach()
            ...
            data_dict[len_queue:len_queue + len(output)] = output.detach()
            sum_temp += len(output)
        # backward
        optimizer.zero_grad()
...
for epoch in range(start_epoch, args.epochs):
    ...
    len_queue = train(epoch, ood_list, len_queue)
    test()
