...
class CLIP_prototype(nn.Module):
    def __init__(self, num_classes=100, layers=8,  type='ViT-B/16', device='cuda', T=0.1):
        ...
        class_name = [imagenet_classes[i] for i in index]
        #self.text_embeddings = self.get_text_embeddings(class_name, imagenet_templates, model)
        prototypes = torch.load('/afs/cs.wisc.edu/u/t/a/taoleitian/private/code/vos/classification/CIFAR/CLIP/feature_mean.pt')
        self.prototypes = nn.Parameter(prototypes, requires_grad=False)
        self.update_factor = 0.99
    ...
    def logits_compute(self, image_embedding):
        prototypes = self.prototypes.T.detach()
        prototypes = prototypes / prototypes.norm(dim=-1, keepdim=True)

        image_embedding = image_embedding / image_embedding.norm()
        logits = image_embedding @ prototypes
        return logits
    ...
    def update_prototype(self, feature, target):
        for index in range(len(target)):
            self.prototypes.data[target[index]] = \
                (1-self.update_factor) * feature[index] + \
                            self.update_factor * self.prototypes.data[target[index]]
    ...
    def forward(self, input, fc=False, target=None):
        if fc==True:
        ...
        #output = self.logits_compute(feature)
            ...
            #self.update_prototype(feature, target)
