...
path = '/nobackup-slow/taoleitian/CLIP_visual_feature/SUN/6/'
if not os.path.exists(path):
    os.makedirs(path)
model = clipnet_ft(num_classes=100, layers=6)
net = model
net = torch.nn.DataParallel(model, device_ids=list(range(4)))
...
train_data_in = dset.ImageFolder(root="/nobackup-slow/dataset/ImageNet_OOD_dataset/SUN",
