...
parser.add_argument('--feat_dim', type=int, default=640, help='Number of ID samples used to generate outliers')
...
train_transform = trn.Compose([
    trn.Resize(size=(288, 288), interpolation=trn.InterpolationMode.BICUBIC),
...
optimizer = torch.optim.SGD([
    {"params":net.fc.parameters(), "lr": state['learning_rate'] * args.decay_rate},
    {"params": net.visual.attnpool.parameters()},
...
def train(epoch, ood_list):
    ...
    loss_avg = 0.0
    lr_reg_loss_avg = 0.0
    data_dict = torch.zeros(num_classes, args.sample_number, args.feat_dim).cuda()
    res = faiss.StandardGpuResources()
    KNN_index = faiss.GpuIndexFlatL2(res, args.feat_dim)
    ...
    for idx, (data, target) in enumerate(train_loader):
        ...
        if sum_temp == num_classes * args.sample_number and epoch < args.start_epoch:
            ...
            new_dis = MultivariateNormal(torch.zeros(args.feat_dim).cuda(), torch.eye(args.feat_dim).cuda())
            ...
            for index in range(num_classes):
                ...
                sample_point = generate_outliers(ID, input_index=KNN_index,
                                                 ...
                                                 cov_mat=args.cov_mat, sampling_ratio=1.0, pic_nums=args.pick_nums, depth=args.feat_dim)
                #end_time = time.time()
