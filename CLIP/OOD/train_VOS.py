...
from CLIP.CLIP_VOS import CLIP_VOS
...
parser.add_argument('--pic_nums', type=int, default=5)
...
net = CLIP_VOS(num_classes=num_classes, layers=num_layers)
...
net.load_state_dict({k.replace('module.', ''): v for k, v in torch.load('/nobackup-slow/taoleitian/model/vos/ImageNet-100/vos/3/ImageNet-100_dense_baseline_dense_epoch_6.pt').items()}, strict=False)
...
def train(epoch, ood_list):
    ...
    #res_bag = [faiss.StandardGpuResources() for idx in range(8)]
    #cpu_index = faiss.IndexFlatL2(512)
    #co = faiss.GpuMultipleClonerOptions()
    #co.useFloat16 = True
    #KNN_index = faiss.index_cpu_to_gpu_multiple_py(res_bag, cpu_index, co)
    for idx, (data, target) in enumerate(train_loader):
        ...
        if sum_temp == num_classes * args.sample_number and epoch < args.start_epoch:
            ...
            temp_precision += 0.001 * torch.eye(512).cuda()
        ...
        #criterion = nn.L1Loss()
        optimizer.zero_grad()
        #label_target = F.one_hot(target, num_classes=num_classes)
