...
def process_args():
    ...
    parser.add_argument('--in_dataset', default="ImageNet-100", type=str, help='in-distribution dataset')
    ...
    '/nobackup-slow/taoleitian/model/ImageNet-100_scratch.pt', help='Method name.')
    ...
    # args.name = '24_06_23:38_SupCon_resnet34_lr_0.5_warm_True_cosine_True_bsz_512_disp_0.5_comp_1_128_temp_0.1_CIFAR-100_pm_0.95'
...
def knn(layer_idx=0, num_classes=100):
    ...
    index.add(ftrain.numpy())
    index_bad = index
    ################### Using KNN distance Directly ###################
    D, _ = index_bad.search(ftest.numpy(), args.K, )
    scores_in = -D[:,-1]
    for ood_dataset, food in food_all.items():
        print(f"Evaluting OOD dataset {ood_dataset}")
        D, _ = index_bad.search(food.numpy(),args.K)
        scores_ood_test = -D[:,-1]
        aurocs, auprs, fprs = [], [], []
...
if __name__ == '__main__':    
    knn(layer_idx=0, num_classes=100)