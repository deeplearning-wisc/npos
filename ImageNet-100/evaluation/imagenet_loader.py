...
class ImageNet(torch.utils.data.Dataset):
    ...
    def __init__(self, transform=None, img_size=64):
        ...
        for idx in range(1, 11):
            # data_file = os.path.join('/nobackup/ImageNet64/', 'train_data_batch_{}'.format(idx))
            data_file = os.path.join('/nobackup-slow/dataset/ImageNet/', 'train_data_batch_{}'.format(idx))
    ...
    def load_image_batch(self, batch_index):
        # data_file = os.path.join('/nobackup/ImageNet64/', 'train_data_batch_{}'.format(batch_index))
        data_file = os.path.join('/nobackup-slow/dataset/ImageNet/', 'train_data_batch_{}'.format(batch_index))
        d = unpickle(data_file)
        x = d['data']
