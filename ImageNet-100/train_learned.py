...
from models.resnet_outliers import SupCEResNet
import os
import time
...
parser.add_argument('--ngpu', default=2, nargs='*', type=int,
...
parser.add_argument('--sample_from', type=int, default=200, help='Number of IDs per class used to estimate OOD data.')
...
args.model_directory = "/nobackup-slow/taoleitian/CIDER_model/ft/checkpoints/{in_dataset}/{name}/".format(in_dataset=args.in_dataset, name=args.name)
...
def main():
    ...
    #model.init_class_prototypes(args, train_loader)
    optimizer = torch.optim.SGD([
                {"params": model.encoder.parameters()},
                #{"params": model.fc.parameters()},
                {"params": model.head.parameters()},
                {"params": model.prototypes, "lr": state['learning_rate']},
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # save checkpoint
        if (epoch + 1) % args.save_epoch == 0 and (epoch + 1) >= 5:
...
def train_epoch(args, train_loader, model, criterion_disp, criterion_comp, optimizer, epoch, log):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        norm = torch.mean(features.norm(dim=-1, keepdim=True))
        ...
        normed_features = features / norm

        disp_loss = criterion_disp(normed_features, target, prototypes=model.prototypes)
        comp_loss = criterion_comp(normed_features, model.prototypes, target)
        ...
        if i % args.print_freq == 0:
            log.debug('Epoch: [{0}][{1}/{2}]\t'
                ...
                epoch, i, len(train_loader), uloss=disp_losses, ploss=comp_losses, knnloss=lr_reg_losses, top1=acc))
