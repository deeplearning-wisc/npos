...
args.model_directory = "/root/autodl-tmp/tlt/CIDER/{in_dataset}/{name}/".format(in_dataset=args.in_dataset, name=args.name)
...
#setup GPU
#args.gpus = list(map(lambda x: torch.device('cuda', x), range(args.gpus)))
...
def set_loader(args):
    ...
    root_dir = '/root/autodl-tmp/imagenet/images/'
    ...
    labeled_trainloader = torch.utils.data.DataLoader(train_data, \
        batch_size=args.batch_size, shuffle=True, num_workers=16, pin_memory=True, drop_last=True)
...
def set_model(args):
    ...
    model.load_state_dict({k.replace('module.', ''): v for k, v in torch.load(
        '/root/autodl-tmp/tlt/CIDER/CIFAR-100/07_11_20:26_SupCon_resnet34_lr_0.01_warm_False_cosine_True_bsz_512_disp_0_comp_1_512_temp_0.1_CIFAR-100_pm_0.95/checkpoint_10.pth.tar').items()}, strict=True)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.save_epoch == 0:
            save_checkpoint(args, model.state_dict(), epoch + 1)
...
def train_epoch(args, train_loader, model, criterion_disp, criterion_comp, optimizer, epoch, log):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        #print(features.shape)
        #print(criterion_disp.prototypes.shape)
